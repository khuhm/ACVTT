...
class RPLHRCT(Dataset):
    ...
    def load_case_ids(self):
        case_ids = sorted(os.listdir(os.path.join(str(self.data_dir), self.mode, '1mm')))
        return case_ids
    ...
    def load_case_data(self, idx):
        ...
        img_hr = np.load(os.path.join(self.data_dir, self.mode, '1mm', case), mmap_mode='r')
        img_hr = np.transpose(img_hr)

        if self.do_fusion:
            img_lr = np.load(os.path.join(self.fusion_data_dir, self.mode, case), mmap_mode='r')
        else:
            img_lr = np.load(os.path.join(self.data_dir, self.mode, '5mm', case), mmap_mode='r')
            img_lr = np.transpose(img_lr)
    ...
    def get_cor_slice(self, data):
        lr_patch_size = (self.patch_size - 1) // 5 + 1
        # padding
        data['img_lr'] = self.slice_padding(data['img_lr'], lr_patch_size)
        data['img_hr'] = self.slice_padding(data['img_hr'], self.patch_size)

        d, h, w = data['img_lr'].shape
        z_s = random.randint(0, d - lr_patch_size)
        y = random.randrange(0, h)
        x_s = random.randint(0, w - self.patch_size)

        z_e = z_s + lr_patch_size
        x_e = x_s + self.patch_size

        z_hr_s = z_s * self.scale
        z_hr_e = (z_e - 1) * self.scale + 1

        slice_lr = data['img_lr'][z_s:z_e, y, x_s:x_e].copy()
        slice_hr = data['img_hr'][z_hr_s:z_hr_e, y, x_s:x_e].copy()
    ...
    def get_sag_slice(self, data):
        ...
        slice_lr = data['img_lr'][z_s:z_e, y_s:y_e, x].copy()
        slice_hr = data['img_hr'][z_hr_s:z_hr_e, y_s:y_e, x].copy()
